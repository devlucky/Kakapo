...
class KakapoServerTests: QuickSpec {
    ...
    override func spec() {
        ...
        describe("#KakapoServer") {
            ...
            it("should call the handler and return the specified object in it when requesting a registered url") {
                let database = KakapoDB()
                database.create(UserFactory.self, number: 20)
                ...
                KakapoServer.get("/users/:id"){ request in
                    info = request.info
                    return database.find(UserFactory.self, id: 2)
                }
                
                NSURLSession.sharedSession().dataTaskWithURL(NSURL(string: "/users/1")!) { (data, response, _) in
                    responseURL = response?.URL
                    bodyDictionary = try! NSJSONSerialization.JSONObjectWithData(data!, options: .MutableLeaves) as? NSDictionary
                }.resume()
                
                expect(bodyDictionary?["firstName"]).toNotEventually(beNil())
                expect(bodyDictionary?["id"]).toEventually(be(2))
